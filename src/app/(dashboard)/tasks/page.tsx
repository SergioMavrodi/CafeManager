...
export default async function TasksPage() {
  ...
  const staffNameByEmail = new Map(
    (staff ?? []).map((s) => [String(s.email).toLowerCase(), s.name])
  )

  function displayName(profile: { email: string; full_name: string | null } | null) {
    if (!profile) return null
    return profile.full_name || staffNameByEmail.get(profile.email.toLowerCase()) || profile.email
  }
  ...
  const rows: TaskRow[] = (tasks ?? []).map((t) => ({
    ...
    assignee_label: displayName(t.assignee),
    assigned_at: t.assigned_at,
    started_at: t.started_at,
    completed_at: t.completed_at,
    completer_label: displayName(t.completer),
  }))
  ...
  const assignees: AssigneeOption[] = (profiles ?? [])
    ...
    .map((p) => ({
      id: p.id,
      label: p.full_name || staffNameByEmail.get(p.email.toLowerCase()) || p.email,
